...
vec3 calculate_directional_light(
   ...
   // combine results
   vec3 ambient = light.ambient * vec3(texture(materaial.diffuse, tex_coord));
   vec3 diffuse = light.diffuse * vec3(texture(material.diffuse, tex_coord));
   vec3 specular = light.specular * vec3(texture(material.specular, tex_coord));
...
vec3 calculate_point_light(
   ...
   // combine results
   vec3 ambient = light.ambient * vec3(texture(material.diffus, tex_coord));
