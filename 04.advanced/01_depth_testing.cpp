/*
  0. This depth test has the possibility of discarding fragments based on the depth
  1. In the depth prediction problems formulated. How about we modify the loss equations to give more
     importance the objects which are more closer to the camera than those which are quite far away?
  2. z-fighting
     How do we solve the z-fighting?
     - make sure the objects are not placed very close to each other
     - move the near plane as far as possible. but this could clip
       the objects which are near
     - use higher precision depth buffer
  3. This has teh power of having only read-only by specifying some flags
*/
